...
#import "StoreFreeContentController.h"
...
@implementation StoreFreeContentController
    ...
    if ((NSNull *)controller == [NSNull null]) 
	{
		switch (page) {
			case VIEW_CONTROLLER_CARTOON:
				controller = [[StoreFreeCartoonViewController alloc] initWithNibName:@"ContentViewController" bundle:nil];
				break;
			case VIEW_CONTROLLER_MAIN:
				controller = [[StoreFreeViewController alloc] initWithNibName:@"ContentViewController" bundle:nil];
				break;				
			case VIEW_CONTROLLER_EPUB:
				controller = [[StoreFreeEpubViewController alloc] initWithNibName:@"ContentViewController" bundle:nil];				
				break;				
		}
...
- (void)viewDidLoad {
    ...
    [APPDELEGATE openAppMenuWithView:self.view];
    
    /*
    {
        //do appmenu
        NSString *appmenu = APPDELEGATE.m_AppMenu;
        if (appmenu != nil && [appmenu length] > 0){
            //NSURL *appURL = [NSURL URLWithString:appmenu];
            //NSString* urlString = [appURL query];
            NSString* urldecode = [appmenu stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
            
            if ([urldecode hasPrefix:@"appmenu?param="] == YES) {
                NSInteger startIndex = [@"appmenu?param=" length];
                NSString* urlParam = [urldecode substringFromIndex:startIndex];
                
                SBJSON *json = [SBJSON new];
                json.humanReadable = YES;
                
                NSDictionary* rootInfo = [json objectWithString:urlParam];
                
                NSString* menuId = getStringValue([rootInfo objectForKey:@"menu_id"]);
                
                if ([menuId isEqualToString:@"main.detail"] == YES)                                     //detail open command
                {
                    NSString* masterNo = getStringValue([rootInfo objectForKey:@"master_no"]);
                    NSString* mainGroup = getStringValue([rootInfo objectForKey:@"main_group"]);
                    NSString* subGroup = getStringValue([rootInfo objectForKey:@"sub_group"]);
                    
                    if ([masterNo length] > 0 && [mainGroup length] > 0 && [subGroup length] > 0){
                        BookDetailViewController* bookDetailViewController;
                        if ([mainGroup isEqualToString:@"CO"] == YES){
                            bookDetailViewController = [BookDetailViewController createWithMasterNumber:masterNo contentType:ContentTypeCartoon subGroup:subGroup];
                        }
                        else if([mainGroup isEqualToString:@"NO"] == YES){
                            bookDetailViewController = [BookDetailViewController createWithMasterNumber:masterNo contentType:ContentTypeEpub subGroup:subGroup];
                        }
    ...
    //test
    /*
    BookDetailViewController* bookDetailViewController = [BookDetailViewController createWithMasterNumber:@"22950" contentType:ContentTypeEpub subGroup:@"GE"];
    if (bookDetailViewController != nil) {
        [self.view addSubview:bookDetailViewController.view];
    }*/
}
...
- (void)requestDataChanged:(NSInteger)tapIndex
{
	...
	if (tapIndex == TAB_INDEX_FREE_STORE) {	
		ContentViewController* controller = [m_ViewControllers objectAtIndex:m_CurentPage];
