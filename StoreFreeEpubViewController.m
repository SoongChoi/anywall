...
@implementation StoreFreeEpubViewController
...
- (void)viewDidLoad {
	...
	if ([UserProfile getLoginState] == YES) {
		[m_Request contentListWithMenuType:BI_MENU_TYPE_FREE mainGroup:BI_MAIN_GROUP_TYPE_NOVEL pageCount:pageCount listCount:listCount orderType:BI_ORDER_TYPE_RECENT viewType:BI_VIEW_TYPE_LIST userNo:[UserProfile getUserNo]];
	}
	else {
		[m_Request contentListWithMenuType:BI_MENU_TYPE_FREE mainGroup:BI_MAIN_GROUP_TYPE_NOVEL pageCount:pageCount listCount:listCount orderType:BI_ORDER_TYPE_RECENT viewType:BI_VIEW_TYPE_LIST];
	}
...
- (BOOL)requestReloadData:(id)sender
{
	...
	if (m_IsDataChanged == YES) 
	{
		...
		if ([UserProfile getLoginState] == YES) {
			[m_Request contentListWithMenuType:BI_MENU_TYPE_FREE mainGroup:BI_MAIN_GROUP_TYPE_NOVEL pageCount:pageCount listCount:listCount orderType:orderType viewType:BI_VIEW_TYPE_LIST userNo:[UserProfile getUserNo]];
		}
		else {
			[m_Request contentListWithMenuType:BI_MENU_TYPE_FREE mainGroup:BI_MAIN_GROUP_TYPE_NOVEL pageCount:pageCount listCount:listCount orderType:orderType viewType:BI_VIEW_TYPE_LIST];
		}
		m_IsDataChanged = NO;
	}	
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex 
{
	...
	if (buttonIndex == 0) {		 // [Recent Order]
		...
		if (m_IsChangedOrder == YES) {
			...
			if ([UserProfile getLoginState] == YES) {
				[m_Request contentListWithMenuType:BI_MENU_TYPE_FREE mainGroup:BI_MAIN_GROUP_TYPE_NOVEL pageCount:pageCount listCount:listCount orderType:BI_ORDER_TYPE_POPULARITY viewType:BI_VIEW_TYPE_LIST userNo:[UserProfile getUserNo]];	
			}
			else {
				[m_Request contentListWithMenuType:BI_MENU_TYPE_FREE mainGroup:BI_MAIN_GROUP_TYPE_NOVEL pageCount:pageCount listCount:listCount orderType:BI_ORDER_TYPE_POPULARITY viewType:BI_VIEW_TYPE_LIST];	
			}
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath 
{
	...
	if (cell == nil) {
		NSArray* cellObjectArray = [[NSBundle mainBundle] loadNibNamed:@"StoreFreeListCellEpub" owner:nil options:nil]; 
