//
//  StorePaidCartoonViewerController.m
//  PlayBook
//
...
@implementation StorePaidCartoonViewController
...
- (void)viewDidLoad {
	...
	if ([UserProfile getLoginState] == YES) {
		[m_Request contentListWithMenuType:BI_MENU_TYPE_CHARGE mainGroup:BI_MAIN_GROUP_TYPE_CARTOON pageCount:pageCount listCount:listCount orderType:BI_ORDER_TYPE_RECENT viewType:BI_VIEW_TYPE_LIST userNo:[UserProfile getUserNo]];
	}
	else {
		[m_Request contentListWithMenuType:BI_MENU_TYPE_CHARGE mainGroup:BI_MAIN_GROUP_TYPE_CARTOON pageCount:pageCount listCount:listCount orderType:BI_ORDER_TYPE_RECENT viewType:BI_VIEW_TYPE_LIST];
	}
...
- (BOOL) requestReloadData:(id)sender
{
	if (m_IsDataChanged == YES) 
	{
		...
		if ([UserProfile getLoginState] == YES) {
			[m_Request contentListWithMenuType:BI_MENU_TYPE_CHARGE mainGroup:BI_MAIN_GROUP_TYPE_CARTOON pageCount:pageCount listCount:listCount orderType:orderType viewType:BI_VIEW_TYPE_LIST userNo:[UserProfile getUserNo]];
		}
		else {
			[m_Request contentListWithMenuType:BI_MENU_TYPE_CHARGE mainGroup:BI_MAIN_GROUP_TYPE_CARTOON pageCount:pageCount listCount:listCount orderType:orderType viewType:BI_VIEW_TYPE_LIST];
		}
		m_IsDataChanged = NO;
	}
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex 
{
	...
	if (buttonIndex == 0) {		 // [Recent Order]
		...
		if (m_IsChangedOrder == YES) {
			...
			if ([UserProfile getLoginState] == YES) {
				[m_Request contentListWithMenuType:BI_MENU_TYPE_CHARGE mainGroup:BI_MAIN_GROUP_TYPE_CARTOON pageCount:pageCount listCount:listCount orderType:BI_ORDER_TYPE_POPULARITY viewType:BI_VIEW_TYPE_LIST userNo:[UserProfile getUserNo]];	
			}
			else {
				[m_Request contentListWithMenuType:BI_MENU_TYPE_CHARGE mainGroup:BI_MAIN_GROUP_TYPE_CARTOON pageCount:pageCount listCount:listCount orderType:BI_ORDER_TYPE_POPULARITY viewType:BI_VIEW_TYPE_LIST];	
			}
...
- (void) clickBtnSearch:(id)sender 
{
	...
	if (searchViewController != nil) {
		[searchViewController setSearchType:SEARCH_TYPE_CARTOON];
...
{
    NSLog(@"command [%d] %@", command, userInfo);
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath 
{
	...
	if (cell == nil) {
		NSArray* cellObjectArray = [[NSBundle mainBundle] loadNibNamed:@"StorePaidListCellCartoon" owner:nil options:nil]; 
...
- (void) loadMore 
{
	...
	if ([UserProfile getLoginState] == YES) {
		[m_Request contentListWithMenuType:BI_MENU_TYPE_CHARGE mainGroup:BI_MAIN_GROUP_TYPE_CARTOON pageCount:pageCount listCount:listCount orderType:ordetType viewType:BI_VIEW_TYPE_LIST userNo:[UserProfile getUserNo]];
	}
	else {
		[m_Request contentListWithMenuType:BI_MENU_TYPE_CHARGE mainGroup:BI_MAIN_GROUP_TYPE_CARTOON pageCount:pageCount listCount:listCount orderType:ordetType viewType:BI_VIEW_TYPE_LIST];
	}
