...
@implementation StorePaidViewController
...
- (void)viewDidLoad {
	...
	[m_Request recommendContentListWithMenuType:BI_MENU_TYPE_CHARGE];		
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath 
{
	...
	static NSString* CellItentifer = @"StorePaidContentCell";
	StorePaidContentCell* cell = (StorePaidContentCell*) [tableView dequeueReusableCellWithIdentifier:CellItentifer];
	
	if (cell == nil) {
		NSArray* cellObjectArray = [[NSBundle mainBundle] loadNibNamed:@"StorePaidContentCell" owner:nil options:nil]; 
		for (id currentObject in cellObjectArray) {
			if ([currentObject isKindOfClass:[StorePaidContentCell class]] == true) {
				cell = (StorePaidContentCell *) currentObject;
				break;
			}
		}
	}
...
- (void) iconImageDidLoad:(NSIndexPath *)indexPath iconImage:(NSData*)iconImage
{
	...
	StorePaidContentCell* cell = (StorePaidContentCell*) [m_TableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
...
- (void) selectedStorePaidContentItem:(id)sender index:(NSInteger)index
{
	NSLog(@"index=[%d]", index);
